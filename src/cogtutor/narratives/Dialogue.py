...
def load_and_chat_auto(
        base_model_path,
        lora_weights_path,
        output_file="conversation_log.json",
        rounds=20,
        chatgpt_identity="You are a helpful assistant..."
):
...
# 示例调用
base_model_path = "qwen-7b/"  # 替换为实际基础模型路径
lora_weights_path = "lora/"  # 替换为实际LoRA权重路径

# 调用函数并将对话内容保存到 'auto_conversation_log.json'
load_and_chat_auto(
    base_model_path,
    lora_weights_path,
    output_file="auto_conversation_log.json",
    rounds=10,  # 对话轮数，最多20轮
    chatgpt_identity="You are a helpful assistant focused on AI and robotics."
)
